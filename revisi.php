		...
		<form id="formorder" action="" method="post" onchange="validateForm('formorder')">
			<div id="box-content">
				...
				<div id="section2" style="">
					<table class="table" style="width: 100%;">
						<tr>
							...
							<div id="masterproduct">
                                ...
                                <div class="hapus_baris" style="position: absolute; display:none; left:-24px; top:43px; color:#fff; background:red; border-radius:50%; padding:0px 6px;">
									...
									<div class="totalprice" style="width:40%;">
                                        ...
                                        <div style="display: flex; justify-content:space-between; flex-wrap:wrap; width:40%; margin:auto; margin-right:0px;">
							...
							<div>
								<button style="background:#fb5; border:none; color:#fff; padding:6px 12px; box-shadow: 2px 2px 2px #aaa;" type="button" name="btn-order" onclick="clone()">New Item +</button>
							</div>
				...
				<div class="button">
					...
					<button id="btn-submit" class="green_bg" type="submit" name="btn-order" disabled>CONFIRM</button>
				</div>
...
function clone(){
	...
	tes.appendChild(cln);
    reset_product(hitung);
	select_product(hitung);
	coba();
}
function reset_product(data){
...
function total_price(data){
    ...
    var nett_price = document.getElementsByClassName("nettprice")[data];
    ...
    nett_price.innerHTML = total_price.value;
    total_all();
}
function total_all(){
    var i = 0;
    var total_price = document.getElementsByClassName("in_totalprice");
    var total = document.getElementById("total_prices");
    var hasil = 0;
    for(i=0; i < total_price.length; i++){
        hasil = parseInt(hasil) + parseInt(total_price[i].value);
    }
...
function unit(data,selected){
    ...
    var hasil = unit[i].split(" :: ");
	tes = tes + 
	"<option value='"+unit[i]+"'>" + hasil[0] + "</option>";
	}
